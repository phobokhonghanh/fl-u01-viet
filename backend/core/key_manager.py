...
from models.schemas import (
    DEFAULT_KEY_PRODUCT,
    KeyRecord,
    is_valid_key_product,
    normalize_key_product,
    normalize_key_level,
    is_valid_key_level,
    DEFAULT_KEY_LEVEL
)
...
def verify_and_get_key(
    ...
    """
    Validate the given key, and return the KeyRecord if it is valid.
    """
...
def check_key(
    ...
    """
    return verify_and_get_key(s3_key, key_str, machine_id, product) is not None
...
def add_or_update_key_by_name(
    ...
    normalized_level = normalize_key_level(level)
    records = load_keys(s3_key)
    for record in records:
        if record.name == name:
            changes: dict = {}
    ...
    for item in new_keys_data:
        try:
            ...
            if not is_valid_key_product(product):
                ...
                continue
            level = normalize_key_level(item.get("level", DEFAULT_KEY_LEVEL))
            if not is_valid_key_level(level):
                logger.warning(f"Nhập key thất bại vì level không hợp lệ: {level}")
                continue
            item = {**item, "product": product, "level": level}
